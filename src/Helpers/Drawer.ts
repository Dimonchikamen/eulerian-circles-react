...
export class Drawer {
        ...
        body.forEach((row) => {
          ...
          if (expressionIsTrue) {
            ...
            for (let k = 0; k < row.length - 1; k++) {
              ...
              subResult =
                subResult &&
                this.getDistance(circle.center, curPoint) < radius ===
                  Boolean(row[k]);
            }
            if (subResult) {
  ...
  private static getCircles(
    ...
    for (let i = 0; i < centres.length; i++) {
      const namePosition = this.getNamePosition(
        centres[i],
        circleRadius,
        width,
        height
  ...
  private static getNamePosition(
    circleCenter: Point,
    radius: number,
    canvasWidth: number,
    canvasHeight: number
